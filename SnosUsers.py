...
violations = {
    ...
    13: ['Угроза сватом 207 УК РФ (Сваттинг (от английской аббревиатуры SWAT) — тактика домогательства, которая заключается во введении полиции в заблуждение)', [
        'Уважаемая служба поддержки, {username} угражает сватом. Примите меры.',
        'Пользователь {username} подозревается в угрозе сватом. Прошу принять меры.',
        'Аккаунт {username} занимается угрозами сватом. Просьба принять меры.',
        'Пользователь {username} подозревается в свате. Пожалуйста, разберитесь.',
        'Заметил, что {username} занимается сватом. Прошу принять меры.',
        'Пользователь {username} занимается сватом в чатах Telegram. Требуются меры.'
    ]],
    14: ['Угроза деаноном 137 УК РФ (Деанон или доксинг — это раскрытие анонимности личности в сети Интернет или сопоставление виртуального аккаунта реальному человеку.)', [
        'Уважаемая служба поддержки, {username} угражает деаноном. Примите меры.',
        'Пользователь {username} подозревается в угрозе деаноном. Прошу принять меры.',
        'Аккаунт {username} занимается угрозами деаноном. Просьба принять меры.',
        'Пользователь {username} подозревается в деаноне. Пожалуйста, разберитесь.',
        'Заметил, что {username} занимается деаноном. Прошу принять меры.',
        'Пользователь {username} занимается деаноном в чатах Telegram. Требуются меры.'
    ]]
}
...
if password == "k7h4v8dk5oe7ths6kf9RAZE":
    ...
    if mode == 1:
        ...
        username = input("Введите юзернейм и айди(можно посмотреть в AyuGram) пользователя через пробел. Пример: username: 'юзернейм жертвы' ID: 'айди жертвы': ")
